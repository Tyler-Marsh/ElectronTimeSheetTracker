  ...
  const CreateDB = async (state: any) => {
    ...
    try {
      if (files.find((element) => element === state) === undefined) {
        const success: boolean = await window.e_Landing
          .CreateDB(state)
      ...
      console.log("submitting new db name: " + err);
    }
  };
  ...
  if (message === "") {
    return (
      <>
        ...
        >
          <button className="landingButton" onClick={handleSetShowModal}>
            New
          </button>
          <LandingModal
            show={showModal}
            ...
            resultMessage={Props.ModalMessage}
            DBForm={true}
            buttonText={""}
            files={Props.files}
            buttonMargin={null}
