...
function Employee(Props: Props) {
    ...
    const Submit = async (): Promise<void> =>  {
        ...
        const anEmployee : EmployeeModel = {Name: state.Name, pk_EmployeeId: 0, fk_DepartmentId: 0, DeleteIndicator: 0 };
        const result : DbSingleResult = await window.e_AddRemove.AddEmployee(settingsStore.SelectedDb, anEmployee);
    ...
    if (state.Step === 1)
    {
        return(
            <>
                <p style={{textAlign: "center", marginTop: "0rem", borderBottom: "1px solid gainsboro"}}>Add an employee</p>
                <div style={{height: "2rem", margin: "0px auto", width: "50%"}}></div>
                <div className="formGroup">
                    <label className="formLabel" htmlFor="Name">Name</label>
                    <input onChange={(e) => {FormChange(e)}} type="text" className="formInput" placeholder="Name" name="Name"></input>
                </div>
    
                <div className="formGroup">
                    <label className="formLabel" htmlFor="Department">Department</label>
                    <select  onChange={(e) => {FormChange(e)}} className="formSelect">
                    <option value={0}>
                            All
                    </option>
