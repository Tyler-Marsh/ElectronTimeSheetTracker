...
function DeleteEmployee(Props: Props) :JSX.Element {
    ...
    const Submit = async (): Promise<void> =>  {
        ...
        const anEmployee : EmployeeModel = {Name: "", pk_EmployeeId: Props.employeeId , fk_DepartmentId: 0, DeleteIndicator: 0 };
        const result : DbSingleResult = await window.e_AddRemove.DeleteEmployee(settingsStore.SelectedDb, anEmployee);
    ...
    if (state.Step === 1)
    {
        return(
            <>
                <p style={{textAlign: "center", marginTop: "0rem", borderBottom: "1px solid gainsboro"}}>Delete an employee</p>
                <div style={{height: "2rem", margin: "0px auto", width: "50%"}}></div>
                <div className="formGroup">
                    <label className="formLabel">Are you sure you want to delete this employee?</label>
                    <label  className="formLabel">{employeeStore.Employees.find(x => x.pk_EmployeeId === Props.employeeId).Name}</label>                
                </div>
                <div className="formGroup">
