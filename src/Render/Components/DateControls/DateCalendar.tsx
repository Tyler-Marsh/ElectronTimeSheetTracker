...
function DateCalendar(Props: Props) {
 ...
 // HERE NEED TO CHECK FOR A PROPER INPUT....
 // moment('5', 'M').format('MMMM');
  const month = moment(Props.month, "M").format("MMMM"); //=== '' ?  check.format('MMMM') : moment.months(parseInt(Props.month)-1);
  const year =  Props.year //=== '' ?   check.format('YYYY') : Props.year;

  // iterate over them by row components


  // function to determine the number of spaces

  // get number of rows necessary for calendar
  const getNumberRows = (firstOfMonth: number, date : string) : number => {
     ...
     const daysLeftInMonth =  moment(date, "MM/DD/YYYY").daysInMonth() -daysInRow1;
     return Math.ceil(daysLeftInMonth/7 + 1);
   }

   function determineClass(arg: number, selectedNumber: number, selectedMonth: number, selectedYear: number, date: string) : string {
    const check = moment(date, "MM/DD/YYYY");
    if (arg === 0) {
      return "blankDate";
    }
    if (parseInt(check.format("YYYY")) === selectedYear && parseInt(check.format("M")) === selectedMonth 
      && arg === selectedNumber) {
      return "selectedDate"
    }
    return "date"
  ...
  const createRows = (date: Moment)  => {
    ...
    const numRows : number = getNumberRows(firstOfMonth, date.format("MM/DD/YYYY"));
  ...
  */
  ...
  // have to match months too
   const rows = RowArray.map((row, index) => {
        return (
          <DateRow setDate={Props.setDate} date={Props.date} selectedYear={parseInt(Props.year)} selectedMonth={parseInt(Props.month)} selectedDay={parseInt(Props.day)} determineClass={determineClass} key={index} days={row} />
        );
   })
  ...
  return (
    <>
            ...
            <div style={{display: "flex", justifyContent:"space-between", alignItems: "flex-end"}}>
              <div onClick={Props.displayMonth} className="hoverCalendarButton" style={{padding: ".25rem", fontSize: "16px", color: "black"}}>
                {month}
              </div>
              <div onClick={Props.displayYear} className="hoverCalendarButton" style={{padding: ".25rem", fontSize: "16px", color: "black"}}>
              {year}
              </div>
            </div>
            <div style={{display: "flex", justifyContent:"space-between", paddingRight: "5px"}}> 
                <button onClick={() => Props.decrementMonth()} className={["calendarButton", "card"].join(" ")}>
                  &lt;
                </button>
                <button onClick={Props.reset} className={["calendarButton", "card"].join(" ")}>
                ↻
                </button>
                <button onClick={() => Props.incrementMonth()} className={["calendarButton", "card"].join(" ")}>
                &gt;
                </button>
            </div> 
          </div>
         
          <table style={{borderCollapse: "collapse", tableLayout:"fixed", width: "300px"}}>  
