...
contextBridge.exposeInMainWorld("e_Landing", {
  ...
  UpdateDB: async function (
    ...
    const result: boolean = await ipcRenderer.invoke("rename-db", [
      oldName,
      filename,
    ]);
    return result;
  ...
  CreateDB: async function (state: any): Promise<boolean> {
    const result: boolean = await ipcRenderer
      .invoke("create-db", state)
...
contextBridge.exposeInMainWorld("e_AddRemove", {
  ...
  ): Promise<DbSingleResult> {
    try {
      const result: DbSingleResult = await ipcRenderer.invoke("add-employee", {
        dbName,
        Employee,
      });
      return result;
  ...
  EditEmployee: async function (
    ...
    try {
      const result: DbSingleResult = await ipcRenderer.invoke("edit-employee", {
        dbName,
        Employee,
      });
      return result;
      ...
      return {
        success: false,
        message: "Failed to edit an employee",
        rowId: 0,
      };
    }
  },
  ...
  DeleteEmployee: async function (
    ...
    try {
      const result: DbSingleResult = await ipcRenderer.invoke(
        "delete-employee",
        { dbName, Employee }
      );
      return result;
...
contextBridge.exposeInMainWorld("e_Shift", {
  AddShift: async function (
    ...
    try {
      const result: DbSingleResult = await ipcRenderer.invoke("add-shift", {
        dbName,
        Shift,
      });
      return result;
  ...
  GetSingleShifts: async function (
    dbName: string,
    StartDate: string,
    EndDate: string,
    EmployeeId: number
  ): Promise<ApiResult<ShiftModel[]>> {
    try {
      const result: ApiResult<ShiftModel[]> = await ipcRenderer.invoke(
        "get-single-shifts",
        { dbName, StartDate, EndDate, EmployeeId }
      );
      return result;
    } catch (err) {
      console.log(err);
      const result: ApiResult<ShiftModel[]> = new ApiResult<any>(
        false,
        [err],
        []
      );
  ...
  EditShift: async function (
    dbName: string,
    Shift: ShiftModel
  ): Promise<DbSingleResult> {
    try {
      const result: DbSingleResult = await ipcRenderer.invoke("edit-shift", {
        dbName,
        Shift,
      });
      return result;
  ...
  DeleteShift: async function (
    ...
    try {
      const result: DbSingleResult = await ipcRenderer.invoke("delete-shift", {
        dbName,
        ShiftId,
      });
      return result;
  ...
  GetSummaryShifts: async function (
    dbName: string,
    Start: string,
    End: string,
    Department: string,
    DepartmentId: number
  ): Promise<ApiResult<SummaryShift[]>> {
    try {
      const result: ApiResult<SummaryShift[]> = await ipcRenderer.invoke(
        "get-summary-shifts",
        { dbName, Start, End, Department, DepartmentId }
      );
      return result;
    } catch (err) {
      console.log(err);
      const result: ApiResult<SummaryShift[]> = new ApiResult<any>(
        false,
        [err],
        []
      );
...
contextBridge.exposeInMainWorld("e_Summary", {
  DownloadSummary: async function (
    ...
    try {
      const result: DbResult = await ipcRenderer.invoke("download", {
        SummaryObject,
        format,
        start,
        end,
...
contextBridge.exposeInMainWorld("e_Settings", {
  ChangeSettings: async function (
    ...
    try {
      const result: DbResult = await ipcRenderer.invoke("change-settings", {
        dbName,
        SettingsJson,
      });
      return result;
