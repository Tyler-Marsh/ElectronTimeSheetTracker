...
ipcMain.handle("init", async (event, arg) => {
  ...
  try {
    ...
    const Departments: DepartmentsModel | DbResult | [] =
      Agent.GetDepartments();
    try {
      // FIRST ERROR EXPECTED
      const jsonFile: string = fs.readFileSync(
        path.join(app.getPath("userData"), "db", arg.split(".")[0] + ".json"),
        "utf-8"
      );
    ...
    if (Array.isArray(Employees) && Array.isArray(Departments)) {
      let SettingsJson = {
        payPeriodStartDate: json?.payPeriodStartDate,
...
import DepartmentsModel, {
  DepartmentModel,
} from "./Render/Models/DepartmentModel";
...
ipcMain.handle("change-settings", async (event, arg) => {
  ...
  try {
    ...
    return {
      success: jsonSuccess,
      message: "successfully changed the settings",
    };
  } catch (err) {
...
ipcMain.handle(
  "add-employee",
  (event, arg: { dbName: string; Employee: EmployeeModel }): DbSingleResult => {
...
ipcMain.handle(
  ...
  (event, arg: { dbName: string; Employee: EmployeeModel }): DbSingleResult => {
    ...
    try {
      ...
      return {
        success: false,
        message: "Failed to delete the emplyee",
        rowId: 0,
      };
    }
  }
...
ipcMain.handle(
  ...
  (
    event,
    arg: { dbName: string; Department: DepartmentModel }
  ): DbSingleResult => {
    const aPath = path.join(app.getPath("userData"), "db", arg.dbName);
    const Agent = new BackAgent(aPath);
    try {
      ...
      return {
        success: false,
        message: "Failed to add a department",
        rowId: 0,
      };
    }
  }
...
ipcMain.handle(
  "add-shift",
  (event, arg: { dbName: string; Shift: ShiftModel }): DbSingleResult => {
    const aPath = path.join(app.getPath("userData"), "db", arg.dbName);
    const Agent = new BackAgent(aPath);
    try {
...
ipcMain.handle(
  ...
  ): ApiResult<ShiftModel[]> => {
    const aPath = path.join(app.getPath("userData"), "db", arg.dbName);
    const Agent = new BackAgent(aPath);

    try {
      const result: Result<ShiftModel[]> = Agent.GetSingleShifts(
        arg.StartDate,
        arg.EndDate,
        arg.EmployeeId
      );
      const theShifts = new ApiResult<ShiftModel[]>(
        true,
        [],
        result.getValue()
      );
...
ipcMain.handle(
  "delete-shift",
  (event, arg: { dbName: string; ShiftId: number }): DbSingleResult => {
...
ipcMain.handle(
  ...
  ): ApiResult<SummaryShift[]> => {
    const aPath = path.join(app.getPath("userData"), "db", arg.dbName);
    const Agent = new BackAgent(aPath);
    const { Start, End, Department, DepartmentId } = arg;
    try {
      const result: Result<SummaryShift[]> = Agent.GetSummaryShifts(
        Start,
        End,
        Department,
        DepartmentId
...
ipcMain.handle(
  ...
  (
    ...
    const options = {
      ...
      buttonLabel: "save",
      filters: [{ name: "comma separated value", extensions: ["csv"] }],
    };
    ...
    for (const key in arg.SummaryObject) {
      reportString += `${arg.SummaryObject[key].Name},${getHoursMinutes(
        arg.SummaryObject[key].RegHours
      )},${getHoursMinutes(
        arg.SummaryObject[key].OvertimeInMinutes
      )},${getHoursMinutes(arg.SummaryObject[key].Total)} \n`;
    }
